...
class GameObject():
    ...
    def image(self, value: str or pygame.Surface):
        if (value is None):
            ...
            return
    @property
    def position(self) -> pygame.math.Vector3:
        return self._position
    @position.setter
    def position(self, value: pygame.math.Vector3):
        if (type(value) == tuple):
            self._position = pygame.math.Vector3(value[0], value[1], value[2])
        else:
    ...
    @property
    def size(self) -> pygame.math.Vector2:
        return self._size
    @size.setter
        ...
        if shouldScale:
            if (horizontalFlip or verticalFlip):
                ...
                try:
                    ...
                    self._positionOffset = pygame.math.Vector2()
                if horizontalFlip:
                    self._positionOffset.x = self._size.x
                if verticalFlip:
                    self._positionOffset.y = self._size.y
                self.position = self._position
                tempSize = pygame.math.Vector2(abs(valueAsV2.x), abs(valueAsV2.y))
                ...
                self.sprite.image = pygame.transform.scale(self.sprite.ORIGINALIMAGE, pygame.math.Vector2(abs(valueAsV2.x), abs(valueAsV2.y)))
    ...
    def _syncOriginalImage(self):
        ...
        self._offset = pygame.math.Vector2(centerBeforeRotate[0] - self.sprite.image.get_rect().center[0], centerBeforeRotate[1] - self.sprite.image.get_rect().center[1])
